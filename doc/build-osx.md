...
NOTE: Building with Qt4 is still supported, however, could result in a broken UI. As such, building with Qt5 is recommended. Qt5 5.7 requires C++11 which Bish Core doesn't fully support yet, Qt5 5.6.2 has some other issues, so make sure to install Qt version < 5.6.2 (5.6.1-1 is recommended).
...
2.  Build Bish Core:
    This will configure and build the headless bish binaries as well as the gui (if Qt is found).
...
4. Enter "bish-qt" as project name, enter src/qt as location
...
Once dependencies are compiled, see [doc/release-process.md](release-process.md) for how the Bish Core
